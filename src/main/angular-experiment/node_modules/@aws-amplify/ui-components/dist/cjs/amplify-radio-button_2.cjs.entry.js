...
const helpers = require('./helpers-62f0d3b2.js');
...
const AmplifyRadioButton = class {
    ...
    render() {
        return (index.h("span", { class: "radio-button" }, index.h("input", { type: "radio", name: this.name, value: this.value, onInput: this.handleInputChange, placeholder: this.placeholder, id: this.fieldId, checked: this.checked, disabled: this.disabled }), index.h("amplify-label", { htmlFor: this.fieldId }, this.label)));
    }
};
...
const AmplifyTOTPSetup = class {
    constructor(hostRef) {
        ...
        this.headerText = core.I18n.get(Translations.Translations.TOTP_HEADER_TEXT);
        this.code = null;
    ...
    async checkContact(user) {
        ...
        try {
            const dataVerifed = await auth.Auth.verifiedContact(user);
            if (!core.isEmpty(dataVerifed)) {
                this.handleAuthStateChange(authTypes.AuthState.SignedIn, user);
            }
            else {
                const newUser = Object.assign(user, dataVerifed);
                this.handleAuthStateChange(authTypes.AuthState.VerifyContact, newUser);
            }
        }
        catch (error) {
    ...
    async verifyTotpToken(event) {
        ...
        try {
            ...
            this.onTOTPEvent(constants.SETUP_TOTP, constants.SUCCESS, user);
        }
        catch (error) {
    ...
    render() {
        return (index.h("amplify-form-section", { headerText: this.headerText, submitButtonText: core.I18n.get(Translations.Translations.TOTP_SUBMIT_BUTTON_TEXT), handleSubmit: event => this.verifyTotpToken(event), loading: this.loading }, index.h("div", { class: "totp-setup" }, index.h("img", { src: this.qrCodeImageSource, alt: core.I18n.get(Translations.Translations.QR_CODE_ALT) }), index.h("amplify-form-field", { label: core.I18n.get(Translations.Translations.TOTP_LABEL), inputProps: this.inputProps, fieldId: "totpCode", name: "totpCode", handleInputChange: event => this.handleTotpInputChange(event) }))));
    }
};
